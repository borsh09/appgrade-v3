...
export function UnifiedCatalog({ category }: { category: string }) {
  ...
  const filtered = items.filter(item => !model || item.modelSlug === model);
  return <>
    <div className="retail-model-selector-wrap">
      ...
      <section ref={modelRailRef} className="retail-model-selector" aria-label="Модели">
        <button type="button" aria-pressed={!model} className={!model ? 'is-active' : ''} onClick={() => setModel('')}>
        ...
        {models.map(([slug, name]) => {
          ...
          return <button type="button" key={slug} aria-pressed={model === slug} className={model===slug ? 'is-active' : ''} onClick={() => setModel(slug)}>
