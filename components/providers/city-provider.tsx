...
interface CityContextValue { cityId: CityId; city: (typeof CITIES)[CityId]; setCityId: (id: CityId) => void }
...
export function CityProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (saved && saved in CITIES) queueMicrotask(() => setCityIdState(saved as CityId));
  }, []);
  const setCityId = (id: CityId) => { setCityIdState(id); localStorage.setItem(STORAGE_KEY, id); };
